<template>
  <div ref="heroSection" class="relative h-[50vh] lg:h-[60vh] flex flex-col justify-center items-center text-white px-4 sm:px-8">
  ...
  <div id="portfolio-section" ref="allProjectsSection" class="max-w-screen min-h-screen flex flex-col justify-center items-center px-4 py-16 sm:px-8 sm:py-24 bg-slate-100">
    <div class="flex flex-col w-full max-w-6xl text-center">
      <p class="text-lg font-semibold text-gray-600">Our Portfolio</p>
      <h1 class="text-4xl md:text-5xl font-bold text-[#EB5523] md:mt-2">All Projects</h1>
    </div>
    <div class="flex flex-col lg:flex-row w-full max-w-6xl mt-12 justify-center items-center">
      <div v-if="portfoliosPending" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
        <SkeletonPortfolioCardSkeleton v-for="i in pageSize" :key="i" />
      </div>
      <div v-else ref="portfolioGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 md:gap-8 gap-3 w-full">
        <portfolio-card
          class="md:h-5/6"
    ...
    <div v-if="!portfoliosPending && portfoliosList.length === 0" class="flex flex-col items-center justify-center py-16">
...
export default {
  ...
  setup() {
    ...
    const { pending: portfoliosPending, data: portfoliosData } = useAsyncData(
      ...
      () => {
        try {
          return $api.get(`/api/portfolios?limit=${pageSize}&page=${currentPage.value}&status=active`).then((res) => res.data);
        } catch (error) {
    ...
    const portfoliosList = computed(() => (portfoliosData.value && portfoliosData.value.data) || []);
    const totalPages = computed(() => (portfoliosData.value && portfoliosData.value.pagination.totalPages) || 1);
