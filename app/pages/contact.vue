<template>
  <div class="flex flex-col max-w-screen overflow-hidden">
    <div ref="heroSection" class="flex flex-col md:flex-row w-full min-h-[50vh] md:h-screen">
      <NuxtImg src="/img/contact-2.png" alt="" class="h-1/2 w-full md:h-full md:w-1/2 object-cover" loading="lazy" placeholder />
    ...
    <div ref="contactSection" class="flex flex-col-reverse md:flex-row w-full min-h-screen justify-center items-center bg-white">
      ...
      <NuxtImg src="/img/contact.png" crossorigin="anonymous" alt="" class="h-1/2 w-full md:h-full md:w-1/2 object-cover" loading="lazy" placeholder />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    initAnimations() {
      ...
      if (heroSection) {
        gsap.from(heroSection.children[0], {
          duration: 1.2,
          autoAlpha: 0,
          x: -100,
          ease: "power3.out",
        });
        gsap.from(heroSection.children[1], {
          duration: 1.2,
          autoAlpha: 0,
          x: 100,
          ease: "power3.out",
      ...
      if (contactSection) {
        animateOnScroll(contactSection.children[0]); // The form container
        animateOnScroll(contactSection.children[1], { delay: 0.2 }); // The image
      }
    },
  },
};
