<template>
  <div class="flex flex-col bg-white space-y-8">
    ...
    <div>
      ...
      <div
        class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-black mt-4"
      >
        <!-- Total Visitor Card -->
        <div
          class="flex flex-col p-6 bg-white border border-gray-200 rounded-lg shadow-sm space-y-2"
        >
          <div
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div
        class="lg:col-span-2 flex flex-col border bg-white border-gray-200 rounded-lg shadow-sm h-96"
      >
        ...
        <div class="flex-grow overflow-auto p-4">
          <table class="table w-full">
            ...
            <tbody>
              <tr
                v-for="portfolio in topContent?.portfolios.slice(0, 5)"
                :key="portfolio._id"
      ...
      <div
        class="flex flex-col border bg-white border-gray-200 rounded-lg shadow-sm h-96 p-4"
      >
        <h3 class="font-semibold text-xl text-gray-800 mb-4">
          Portfolio Views (Last 7 Days)
        </h3>
        <div class="flex-grow">
          <bar-portfolio
            v-if="portfolioBarData"
            :chart-data="portfolioBarData"
            :max-data="maxDataPortfolio"
          />
        </div>
      </div>
    </div>
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      >
        ...
        <div class="flex-grow overflow-auto p-4">
          <table class="table w-full">
            ...
            <tbody>
              <tr
                v-for="blog in topContent?.blogs.slice(0, 5)"
                :key="blog._id"
                class="border-b border-gray-100"
              >
      ...
      >
        ...
        <div class="flex-grow">
          <bar-portfolio
            v-if="articleBarData"
            :chart-data="articleBarData"
            :max-data="maxDataArticle"
          />
        </div>
      </div>
    </div>
    ...
    async fetchStatisticData() {
      try {
        ...
        const maxPortfolio = Math.max(
          ...(this.portfolioBarData?.datasets[0]?.data || [0])
        );
        ...
        const maxArticle = Math.max(
          ...(this.articleBarData?.datasets[0]?.data || [0])
        );
