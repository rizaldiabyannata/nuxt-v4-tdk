  ...
  <div class="space-y-8">
    ...
    <div v-if="tampilanAktif === 'daftar'" class="space-y-8">
      <div class="flex flex-row justify-between items-center w-full">
        ...
        <div class="flex flex-row items-center space-x-4">
          <label class="input input-bordered flex items-center gap-2 rounded-lg">
            <input type="text" class="grow" placeholder="Search" v-model="filter.search" />
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-4 h-4 opacity-70"><path fill-rule="evenodd" d="M9.965 11.026a5 5 0 1 1 1.06-1.06l2.755 2.754a.75.75 0 1 1-1.06 1.06l-2.755-2.754ZM10.5 7a3.5 3.5 0 1 1-7 0 3.5 3.5 0 0 1 7 0Z" clip-rule="evenodd" /></svg>
          </label>
          <select class="select select-bordered rounded-lg" v-model="filter.status" @change="onFilterChange">
      ...
      <div class="space-y-4">
        ...
        <div v-if="portoHighlightList && portoHighlightList.length > 0" class="grid gap-6 w-full bg-gray-50 rounded-lg border border-gray-200 p-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          <PortoCardAdmin
        ...
        <div v-else class="flex items-center justify-center w-full h-48 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50">
    ...
    <div v-else class="bg-white border border-gray-200 p-8 rounded-lg shadow-sm">
      <form @submit.prevent="isEditing ? updatePortfolio() : submitPortfolio()" class="space-y-6">
        <div>
          <label for="title" class="block text-lg font-medium text-gray-700">Portfolio Title</label>
          <input v-model="portfolio.title" type="text" id="title" class="mt-1 input input-bordered w-full" required />
        </div>
          ...
          <label for="cover-image-upload" class="relative flex flex-col items-center justify-center w-full h-48 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition-colors">
            <div class="text-center">
              ...
              <p class="mt-2 text-sm text-gray-600">
                <span class="font-semibold">{{ portfolio.coverImage ? portfolio.coverImage.name : (isEditing ? 'Choose new image (optional)' : 'Upload a file') }}</span>
              </p>
        ...
        <div>
          <label for="shortDescription" class="block text-lg font-medium text-gray-700">Short Description</label>
          <textarea v-model="portfolio.shortDescription" id="shortDescription" rows="3" class="mt-1 textarea textarea-bordered w-full" required></textarea>
        </div>
...
export default {
  ...
  methods: {
    resetForm() {
      ...
      this.portfolio = { slug: null, title: "", shortDescription: "", description: "", coverImage: null };
      this.existingImageUrl = null;
    },
    async fetchPortfolios() {
      const { search, status } = this.filter;
      let statusFilter = status === 'all' ? '' : (status === 'archive' ? 'archived' : 'active');
      try {
        let apiUrl = `/api/portfolios?limit=10&page=1&status=${statusFilter}`;
        if (search) {
    ...
    async updatePortfolio() {
      ...
      try {
        await this.$api.put(`/api/portfolios/${this.portfolio.slug}`, formData, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        this.$toast?.success?.("Portfolio updated successfully.");
        this.resetForm();
        await this.fetchPortfolios();
        await this.fetchHighlighted();
      } catch (error) {
    ...
    async updatePortfolioStatus(slug, status) {
      try {
        ...
        this.$toast?.success?.(`Portfolio successfully ${action}d.`);
        await this.fetchPortfolios();
        await this.fetchHighlighted();
      } catch (error) {
        console.error(`Failed to ${status} portfolio:`, error);
        this.$toast?.error?.(`Failed to ${status} portfolio.`);
      }
    },
    async sendHighlight(portfolioId) {
      ...
      try {
        await this.$api.post("/api/content-tracking/highlighted-portfolios", { portfolioId }, { headers: { "Content-Type": "application/json" } });
        this.$toast?.success?.("Portfolio highlighted.");
        await this.fetchPortfolios();
        await this.fetchHighlighted();
      } catch (err) {
        console.error("Failed to highlight portfolio:", err);
        this.$toast?.error?.("Failed to highlight portfolio.");
      } finally {
    ...
    async deletePortHighlight(portfolioId) {
      ...
      this.loadingHighlights = portfolioId;
      try {
        await this.$api.delete(`/api/content-tracking/highlighted-portfolios/${portfolioId}`);
