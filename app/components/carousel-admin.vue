<template>
  <div
    class="w-full max-w-xs sm:max-w-sm bg-[#F5EE9E] shadow-lg rounded-2xl flex flex-col p-6 relative"
  >
    <!-- Image Preview (optional, bisa ditambahkan) -->
    <MinioImage
      v-if="imageUrl"
      :src="imageUrl"
      :alt="title"
    ...
    <div class="card-actions justify-start mt-4 flex flex-row space-x-1">
      <button
        @click="edit"
        class="bg-blue-700 p-1 rounded-lg cursor-pointer hover:opacity-50"
      >
        <svg
          width="20"
          height="20"
      ...
      <button
        ...
        class="bg-[#F43232] p-1 rounded-lg cursor-pointer hover:opacity-50"
      >
        <svg
          width="20"
          height="20"
      ...
      <button
        @click="highlightThis"
        class="bg-[#FFCC00] p-1 rounded-lg cursor-pointer hover:opacity-50 items-center aspect-square"
...
export default {
  ...
  methods: {
    ...
    deleteHighlight() {
      ...
      if (!this.slug) {
        console.error(
          "❌ Article Hightlight ID kosong, tidak mengirim ke parent"
        );
        return;
      }
      this.$emit("unhighlight", this.id);
