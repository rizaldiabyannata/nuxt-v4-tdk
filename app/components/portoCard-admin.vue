<template>
  <div class="card bg-base-100 w-full shadow-lg rounded-2xl overflow-hidden flex flex-col aspect-[3/4] min-h-[320px]">
    <div class="card-body rounded-t-2xl p-4 sm:p-6 flex flex-col justify-between" :class="isArchived ? 'bg-gray-300' : 'bg-white'">
      <h2 id="title" class="card-title text-black text-xl font-semibold mb-2 w-full text-wrap">
        {{ title }}
      </h2>
      <p id="shortDescription" class="text-black text-sm sm:text-base mb-2 line-clamp-2">{{ (shortDescription || "").substring(0, 50) }}...</p>
      <div class="card-actions flex flex-row gap-2 mt-2">
        <!-- edit -->
        <button @click="edit" class="bg-blue-700 p-2 rounded-lg cursor-pointer hover:opacity-80 transition">
        ...
        <button @click="deleteCard" class="bg-[#F43232] p-2 rounded-lg cursor-pointer hover:opacity-80 transition">
          <svg width="20" height="20" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
        ...
        <button @click="toggleArchive" class="p-2 rounded-lg cursor-pointer hover:opacity-80 transition" :class="isArchived ? 'bg-green-500' : 'bg-[#797979]'">
        ...
        <button @click="toggleHighlight" class="bg-[#FFCC00] p-2 rounded-lg cursor-pointer hover:opacity-80 transition items-center aspect-square" :disabled="isHighlightLoading">
          <span v-if="isHighlightLoading" class="loading loading-spinner loading-sm"></span>
          <svg v-else width="19" height="17" viewBox="0 0 18 17" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
    ...
    <figure class="h-[40%] w-full flex items-center justify-center bg-gray-100 rounded-b-2xl overflow-hidden">
      <img crossorigin="anonymous" id="image" :src="imageUrl" alt="Product Image" class="w-full h-full object-cover" />
    </figure>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    toggleArchive() {
      if (!this.slug) {
        console.error("❌ Portfolio SLUG kosong, tidak dapat mengubah status arsip");
        return;
      }
      if (this.isArchived) {
