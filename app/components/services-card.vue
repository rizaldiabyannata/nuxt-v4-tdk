<template>
  <div class="flex flex-col rounded-4xl p-8 bg-white border border-gray-300 hover:shadow-lg transition-shadow h-full">
    <!-- Image and Title -->
    <div class="flex flex-col w-full items-center justify-center mb-4">
      <NuxtImg 
        ...
        loading="lazy"
      />
      <p class="font-extrabold text-black text-xl lg:text-2xl text-center">{{ title }}</p>
    </div>
    
    <!-- List with Dividers -->
    <div class="flex flex-col w-full space-y-2 flex-1">
      <div v-for="(item, idx) in list" :key="idx" class="flex flex-col w-full">
    ...
    <div class="pt-6 mt-4 border-t border-gray-300">
      <NuxtLink 
        to="/contact" 
        class="flex flex-row w-fit border-black border rounded-full px-4 py-2 text-sm items-center space-x-2 hover:bg-gray-100 transition-colors"
      >
